...
{
    $start = microtime(true); // For testing response time
    ...
    switch ($currentRouteName) {
        case 'ecomm.products':
            ...
            switch ($request->type) {
                case 'tranding': case 'featured': case 'hot-product':
                    $rules['category_id'] = 'required|integer';
                    $is_trn = $request->type == 'tranding';
                    $is_fea = $request->type == 'featured';
                    $is_hot = $request->type == 'hot-product';
                    break;
            }
            break;
        case 'ecomm.hot-deals-product': $is_hot = true; break;
        case 'ecomm.tranding-product': $is_trn = true; break;
        case 'ecomm.search-product': $search = $request->string; break;
        case 'ecomm.featured-product': $is_fea = true; break;
        case 'ecomm.related-product':
        case 'ecomm.category-product': $rules['category_id'] = 'required|integer'; break;
    ...
    // Fetch products (should be optimized to paginate + eager load)
    ...
    $wishlists = $user_id ? Wishlist::whereIn('product_id', $productIds)->where('user_id', $user_id)->get()->keyBy('product_id') : collect();
    $ratings = ProductRating::whereIn('product_id', $productIds)->get()->groupBy('product_id');
    ...
    foreach ($products as $product) {
        $wishlist = $wishlists->get($product->id);
        $wish_status = $wishlist ? 1 : 0;
        $wishlist_id = $wishlist ? $wishlist->id : null;

        $cart = $carts->get($product->id);
        $cart_type_name = $cart && $cart->type ? ($lang != "hi" ? $cart->type->type_name : $cart->type->type_name_hi) : '';
        $cart_type_price = $cart->type_price ?? null;
        $cart_quantity = $cart->quantity ?? null;
        $cart_total_price = $cart->total_qty_price ?? null;
        $cart_status = $cart ? 1 : 0;

        $ratingData = $ratings->get($product->id);
        $rating_avg = $ratingData ? number_format($ratingData->avg('rating'), 1, '.', '') : 0;
        $total_reviews = $ratingData ? $ratingData->count() : 0;

        // Fetch types with optional cache
        $typedata = Cache::remember("typedata_{$product->id}_{$roleType}_{$state_id}_{$city_id}_{$lang}", 60, function () use ($product, $roleType, $state_id, $city_id, $lang, $type_id) {
            return $this->fetchProductTypes($product->id, $roleType, $state_id, $city_id, $lang, $type_id);
        });
        ...
        if (!empty($typedata) && isset($typedata[0]['type_name'])) {
            $selected = $typedata[0];
            $selected_range = $selected['range'][0] ?? [];
            $selected_type_id = $selected['type_id'] ?? '0';
            $selected_type_name = $selected['type_name'] ?? 'Def';
            $selected_type_selling_price = $selected_range['selling_price'] ?? 0;
            $selected_type_mrp = $selected_range['type_mrp'] ?? 0;
            $selected_type_percent_off = $selected_range['percent_off'] ?? 0;
            $selected_min_qty = $selected['min_qty'] ?? 0;

            if ($roleType == 2) {
                $vendorSelectedType = vendorType::where('type_name', $selected_type_name)->where('id', $selected_type_id)->first();
    ...
    \Log::info('Execution Time: ' . (microtime(true) - $start) . ' Seconds');
