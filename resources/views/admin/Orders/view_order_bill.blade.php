    ...
    <div class="container">
      <div class="row">
        ...
        <div class="col-sm-6 billing_content">
          ...
          {{$address->doorflat ?? ''}}{{ $address->address ?? '' }},{{$address->landmark ?? ''}},{{$address->citys->city_name ?? ''}},{{$address->states->state_name ?? ''}},{{$address->zipcode ?? ''}}<br>
          State/UT Code: RJ<br>
      ...
      <div class="row">
        ...
        <div class="col-sm-6 invoice_content">
          Invoice No: OSPL/24-25/{{ $invoice->invoice_no < 10 ? '0'.$invoice->invoice_no : $invoice->invoice_no ?? '00' }}<br>
        </div>
      </div>
    </div>
        ...
        <tbody>
          ...
          @php $total_tax += $item->type->gst_percentage_price * $item->quantity @endphp
            <tr class="product_table2">
              ...
              <td>Rs. {{ $item->amount + ($item->type->gst_percentage_price * $item->quantity) / 2 + ($item->type->gst_percentage_price * $item->quantity) / 2 }}</td>
            </tr>
