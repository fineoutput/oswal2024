...
<div class="details-product-content">
    ...
    <div class="details-purchase-info">
        ...
        @else
        <button type="button"  id="addtocartforview{{ $product->id }}" class="details-btn" data-url="{{ route('cart.get-cart-details') }}" onclick="manageCart({{ $product->id }}); anotherFunction({{ $product->id }});">Add to Cart <i class="fas fa-shopping-cart"></i></button>
